---
title: "Upgrade PGO 3.5 to 4.2.2"
Latest Release: 4.2.2 {docdate}
draft: false
weight: 8
---

## Upgrading a Cluster from Version 3.5.x to PGO 4.2.2

This section will outline the procedure to upgrade a given cluster created using Postgres Operator 3.5.x to PGO version 4.2.2. This version of the Postgres Operator has several fundamental changes to the existing PGCluster structure and deployment model. Most notably, all PGClusters use the new Crunchy Postgres HA container in place of the previous Crunchy Postgres containers. The use of this new container is a breaking change from previous versions of the Operator.
...
* The latest PGO 4.2.2 code for the Postgres Operator available
* The latest PGO 4.2.2 client binary

##### Step 0
Create a new Linux user with the same permissions are the existing user used to install the Crunchy Postgres Operator. This is necessary to avoid any issues with environment variable differences between 3.5 and 4.2.2.
...
##### Step 3
Log in as your new Linux user and install the 4.2.2 Postgres Operator.
...
##### Step 5
Manually update the old leftover Secrets to use the new label as defined in 4.2.2:
