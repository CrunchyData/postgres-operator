...
You can also scale down replicas as of Operator version 3.2, to list
the replicas for a given cluster, you enter the following:

....
pgo scale mycluster --query
....

Then, using a replica name from the query, you can remove that
replica using this command:
....
pgo scale mycluster --scale-down-target=mycluster-replica-xxxx
....

You can also delete the PVC and its data for the scaled down replica
...
=== pgo show user
pgo managed sser credentials are generated through Kubernetes Secrets automatically for the *testuser*, *primaryuser* and *postgres* accounts. The generated passwords can be viewed as following:
....
pgo show user all 
pgo show user --selector=name=mycluster 
....

For this command to work, you will need the SHOW_SECRETS_PERM permission
defined for your user role.
