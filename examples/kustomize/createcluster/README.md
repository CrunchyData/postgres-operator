...
```
cluster : hippo (crunchy-postgres-ha:centos8-13.6-4.7.5)
	...
	labels : pgo-version=4.7.5 name=hippo crunchy-pgha-scope=hippo deployment-name=hippo pg-cluster=hippo pgouser=admin vendor=crunchydata
```
...
You will see something like this if successful:
```
cluster : dev-hippo (crunchy-postgres-ha:centos8-13.6-4.7.5)
	...
	labels : crunchy-pgha-scope=dev-hippo name=dev-hippo pg-cluster=dev-hippo vendor=crunchydata deployment-name=dev-hippo environment=development pgo-version=4.7.5 pgouser=admin
```
#### staging
...
```
cluster : staging-hippo (crunchy-postgres-ha:centos8-13.6-4.7.5)
	...
	labels : deployment-name=staging-hippo environment=staging name=staging-hippo crunchy-pgha-scope=staging-hippo pg-cluster=staging-hippo pgo-version=4.7.5 pgouser=admin vendor=crunchydata
```
...
You will see something like this if successful, (Notice one of the replicas is a different size):
```
cluster : prod-hippo (crunchy-postgres-ha:centos8-13.6-4.7.5)
	...
	labels : pgo-version=4.7.5 deployment-name=prod-hippo environment=production pg-cluster=prod-hippo crunchy-pgha-scope=prod-hippo name=prod-hippo pgouser=admin vendor=crunchydata
```
### Delete the clusters
