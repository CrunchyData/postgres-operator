...
```
curl --cacert $PGO_CA_CERT --key $PGO_CLIENT_KEY --cert $PGO_CA_CERT -u pgoadmin:examplepassword -H "Content-Type:application/json" --insecure -X GET $PGO_APISERVER_URL/version
```

#### Body examples
In the following examples data is being passed to the apiserver using a json structure. These json structures are defined in the following documentation.

```
curl --cacert $PGO_CA_CERT --key $PGO_CLIENT_KEY --cert $PGO_CA_CERT -u pgoadmin:examplepassword -H "Content-Type:application/json" --insecure -X GET "$PGO_APISERVER_URL/workflow/af7d488e-b039-4b33-8e40-087911ddb799?version=<client-version>&namespace=<namespace>"
```
...
```
curl --cacert $PGO_CA_CERT --key $PGO_CLIENT_KEY --cert $PGO_CA_CERT -u pgoadmin:examplepassword -H "Content-Type:application/json" --insecure -X POST --data \
    '{"ClientVersion":"4.1.0",
    "Namespace":"pgouser1",
    "Name":"mycluster",
    "Series":1}' \
