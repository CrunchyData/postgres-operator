...
func Ls(request *msgs.LsRequest, ns string) msgs.LsResponse {
	resp := msgs.LsResponse{}
	resp.Status.Code = msgs.Ok
	resp.Status.Msg = ""
	resp.Results = make([]string, 0)

	log.Debugf("Ls %v", request)
	...
	clusterName = request.Args[0]
	log.Debugf("ls called for cluster %s", clusterName)

	var results string
	results, err = ls(&podList.Items[0], apiserver.Clientset, apiserver.RESTClient, apiserver.RESTConfig, ns, request.Args)
	if err != nil {
		resp.Status.Code = msgs.Error
		resp.Status.Msg = err.Error()
		return resp
...
// run ls.sh on the postgres pod, remember we are assuming
...
//make sure the parameters to the ls command dont' container mischief
