...
`)
	...
	t.Run("InstrumentationSpecDefinedDetectorSet", func(t *testing.T) {
		...
		instrumentationSpec.Config.Detectors = []v1beta1.OpenTelemetryResourceDetector{
			{
				Name: "gcp",
			},
		}
		config := NewConfigForPgBackrestRepoHostPod(ctx, instrumentationSpec, repos, "/another/directory")
