...
[Upgrade Postgres Operator from 4.X to 4.2.4 (Bash)] ( {{< relref "upgrade/upgrade4xto42_bash.md" >}})

[Upgrade Postgres Operator from 4.X to 4.2.4 (Ansible)] ( {{< relref "upgrade/upgrade4xto42_ansible.md" >}})
...
In this example, we are upgrading a cluster from PostgreSQL 11.6 to 11.9 using the `crunchy-postgres:centos7-11.9-4.2.4` container:

`pgo upgrade mycluster --ccp-image-tag=centos7-11.9-4.2.4`
