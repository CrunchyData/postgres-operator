---
title: "Upgrade PGO 4.X to 4.2.4 (Bash)"
Latest Release: 4.2.4 {docdate}
draft: false
weight: 8
---

## Postgres Operator Bash Upgrade Procedure from 4.X to 4.2.4

This procedure will give instructions on how to upgrade to version 4.2.4 of the Crunchy Postgres Operator using the Bash installation method. This version of the Postgres Operator has several fundamental changes to the existing PGCluster structure and deployment model. Most notably, all PGClusters use the new Crunchy Postgres HA container in place of the previous Crunchy Postgres containers. The use of this new container is a breaking change from previous versions of the Operator.
...
* The latest 4.2.4 PGO client binary
...
```
There is a new eventing feature in 4.2.4, so if you want an alias to look at the eventing logs you can add the following:
```
elog () {
...
Ensure you have checked out the latest 4.2.4 version of the source code and update the pgo.yaml file in `$PGOROOT/conf/postgres-operator/pgo.yaml`

You will want to use the 4.2.4 pgo.yaml file and update custom settings such as image locations, storage, and resource configs.
...
##### Step 7
Next, update the PGO client binary to 4.2.4 by replacing the existing 4.X binary with the latest 4.2.4 binary available.
...
##### Step 9
The Operator is now upgraded to 4.2.4 and all users and roles have been recreated.
