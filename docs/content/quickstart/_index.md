...
```
curl https://raw.githubusercontent.com/CrunchyData/postgres-operator/v4.3.1/installers/kubectl/postgres-operator.yml > postgres-operator.yml
```
...
```
curl https://raw.githubusercontent.com/CrunchyData/postgres-operator/v4.3.1/installers/kubectl/client-setup.sh > client-setup.sh
...
Note that the `pgo` client's version must match the version of the PostgreSQL Operator that you have deployed. For example, if you have deployed version 4.3.1 of the PostgreSQL Operator, you must use the `pgo` for 4.3.1.
