configMap= PostgreSQL Operator Documentation
:toc:
v2.6, {docdate}
...
either a DNS name for the postgres-operator service or its service
IP address.
...
privileges using the cluster role binding *postgres-operator-cluster-role-binding*.
...
the *apiserver*.  Sample keys and certificates used by TLS are found
here:
....
...
Kubernetes resources and require privileges for interacting with Kubernetes.
The `rbac.yaml` file includes a set of roles and bindings
that allow the operator to work.  These are fine grained
controls that you can adjust within your local Kubernetes cluster depending
...
instead of the Kubernetes API.  Therefore, it requires its own RBAC
configuration.

Starting in Release 2.6, the file `$COROOT/conf/apiserver/pgorole` is used to
define some sample pgo roles - *pgadmin* and *pgoreader*.

These roles are meant as examples that you can configure to suit
...
Also, the `pgouser` file now includes the role that is assigned to
...
NOTE: The default `pgo.yaml` file assumes you are going to use *HostPath* Persistent Volumes for
...
More in-depth explanations of possible configurations are available below.
...
sure to set the environment variable `CO_APISERVER_URL` to either the DNS
name or IP address of the *postgres-operator* service.  For example:
...
This section describes the configuration options for the PostgreSQL operator.
...
and get added into a ConfigMap which is mounted by the operator deployment.
...
== pgo Commands
link:/commands.asciidoc[this is the commands link]
...
In the example above, 3 clusters are provisioned that have a number appended
into their resulting cluster name. Then, a user defined label is applied to each
cluster, and user defined policies are applied to each cluster after they are created.
...
Also, in order to view JSON formatted output, add the `-o json` flag:
....
...
NOTE: User credentials are generated through Kubernetes Secrets automatically for the
*testuser*, *primaryuser* and *postgres* accounts. The generated passwords can be viewed
by running the `pgo show cluster` command with the `--show-secrets` command. More details
...
You can filter the results based on the PostgreSQL Version:
....
...
NOTE: This command will not remove the PVC associated with
this cluster.
...
to make sure this is what you want to do. To prevent the prompt,
enter the `--no-prompt` command line flag.
...
deployment is set to a replica count of 1 by default and it can not scale beyond that.

With PostgreSQL, you can any n-number of replicas each of which
connect to the primary forming a streaming replication PostgreSQL cluster.
The replicas are read-only, whereas the primary is read-write.
To create a replica enter a command such as:
....
pgo scale mycluster
....
...
can be used to influence what Kubernetes node the replica will be scheduled
upon.
...
be scheduled on a node that the primary is not running on.
...
You can perform a minor PostgreSQL version upgrade
...
and recreate them using the currently defined PostgreSQL
container image specified in your pgo configuration file.

The database data files remain untouched, only the container
is updated, this will upgrade your PostgreSQL server version only.

You can perform a major PostgreSQL version upgrade
of either a database or cluster as follows:
....
...
The database data files are converted to the new major PostgreSQL
version as specified by the current image version
in your pgo configuration file.

In this scenario, the upgrade is performed by the
...
using the existing database files as input, and output
...
You can get output using the `--output` or `-o` flag:
....
pgo test all -o json
....
...
using the file `/tmp/policy1.sql` as input.  It will create
...
is located in the repository at `$COROOT/examples/pgo-bash-completion`. To use it:
....
sudo cp $COROOT/examples/pgo-bash-completion /etc/bash_completion.d/pgo
su - $USER
....

== REST API

Because the apiserver implements a REST API, it is possible to integrate
customized application code with the apiserver. To demonstrate this, the
...
created as part of a PostgreSQL cluster.  Running pgpool only
makes sense when you have both a primary and some number of
replicas deployed as part of your PostgreSQL cluster.  The current
...
configuration files specifically for this PostgreSQL cluster.
