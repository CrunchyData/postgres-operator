			...
			if arg == "all" || database.Spec.Name == arg {
				if PostgresVersion == "" || (PostgresVersion != "" && database.Spec.POSTGRES_FULL_VERSION == PostgresVersion) {
					...
					if err != nil {
						...
						fmt.Println(TREE_BRANCH + "pod : " + pod.Name + " (" + string(pod.Status.Phase) + ")" + " (" + getReadyStatus(pod) + ")" + " (" + database.Spec.POSTGRES_FULL_VERSION + ")")
					}
					...
					if ShowSecrets {
						showSecrets(database.Spec.Name)
					}
				}
			}
		}
