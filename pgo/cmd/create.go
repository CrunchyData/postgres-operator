...
var CreateCmd = &cobra.Command{
	...
	Long: `CREATE allows you to create a new PostgreSQL Cluster, Policy, or User. For example:
...
var createClusterCmd = &cobra.Command{
	...
	Long: `Create a PostgreSQL cluster which consists of a
...
pgo create cluster mycluster`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if SecretFrom != "" || BackupPath != "" || BackupPVC != "" {
			if SecretFrom == "" || BackupPath == "" || BackupPVC == "" {
				log.Error("The --secret-from, --backup-path, and --backup-pvc command line flags are all required to perform a restore.")
				return
			}
		}
...
pgo create policy mypolicy --in-file=/tmp/mypolicy.sql`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if PolicyFile == "" && PolicyURL == "" {
			log.Error("The --in-file or --url flags are required to create a policy.")
			return
		}
...
pgo create user user1 --selector=name=mycluster`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if Selector == "" {
			log.Error("The --selector flag is required to create a user.")
			return
		}
